...
def detect_interface_ips(interfaces: List[str]) -> dict:
    results: Dict[str, str | None] = {interface: None for interface in interfaces}
    for interface in interfaces:
        try:
            ...
            if output:
                # Beispiel: 2: eth0    inet 192.168.4.2/24 brd ...
                inet_part = output.split("inet ", 1)[1]
                results[interface] = inet_part.split("/", 1)[0].strip()
        except Exception:
            results[interface] = None
    return results


def build_network_status(port: int) -> dict:
    interface_ips = detect_interface_ips(["eth0", "wlan0"])
    urls = {
        name: (f"http://{ip}:{port}" if ip else None)
...
class RobotRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if path == "/":
            ...
            self._send_json(build_network_status(port))
...
def run() -> None:
    ...
    network_status = build_network_status(port)
    print(f"Server läuft auf http://{host}:{port} (Ethernet + WLAN freigegeben)")
    for interface_name in ["eth0", "wlan0"]:
        interface_url = network_status["urls"].get(interface_name)
        if interface_url:
